...
class Attention(nn.Module):
    """
    输入：(B, N, C)，patch embedding序列
    输出：(B, N, C)，经过attention后的特征

    标准的Transformer多头自注意力机制
    """
...
class Block(nn.Module):
    """
    输入：(B, num_patches*T + cls_token, embed_dim)
    输出：同样尺寸的特征

    首先在时间维度上进行注意力计算，接着在空间维度上进行注意力计算，最后通过MLP进行进一步特征融合
    """
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.patch_size = patch_size  # 每个patch的embedding维度
        self.num_patches = num_patches  # 每帧的patch数量
...
class VisionTransformer(nn.Module):
    """
    Vision Transformer 主体，集成了所有子模块

    输入：视频 (B, C, T, H, W)
    输出：特征 (B, embed_dim)

    执行完整的视频特征提取流程，包括Patch嵌入、空间和时间位置嵌入以及多层的时空Transformer block处理
    """
...
class TimeSformer(nn.Module):
    """
    输入：(B, C, T, H, W)
    输出：视频的特征表示 (B, embed_dim)

    整体封装了VisionTransformer，实现完整的视频特征抽取流程，并支持加载预训练模型
    """
...
def build_vit_base_patch16_224():
    ...
    opt = parse_args()
    opt.cfg_file = "configs/TimeSformer_divST_8x32_224.yaml"
